...
var $ = require("jquery");
var separator_class_name = '.prob-separator';
var separator_data_attr_name = 'data-separator';
var Separator = (function () {
    function Separator(_label, _connector) {
        ...
        this.separator.on('mousedown', function (e) {
            self.dragging = true;
            self.px = e.pageX;
        ...
        $(document).on('mousemove', function (e) {
            if (!self.dragging)
                return;
            self.px = e.pageX;
            var prevTargetWidth = e.pageX - self.getOriginX();
            var nextTargetWidth = self.PrevNextOriginalMergedWidth - prevTargetWidth;
            if (prevTargetWidth <= 0)
                return;
            if (nextTargetWidth <= 0)
                return;
    ...
    Separator.prototype.getOriginX = function () {
        var base_probbox = $(separator_class_name + '[' + separator_data_attr_name + '=' + this.label + ']').prev();
        return base_probbox.offset().left;
    };
    Separator.prototype.getEntropy = function () {
        var self = this;
        var entropy = 0;
        self.separator.parent().find('.each-prob').each(function (i, el) {
            var prob = $(el).width() / self.probbarWholeWidth;
